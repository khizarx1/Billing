...
export default function Signup() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const { data } = await axios.post('http://localhost:3000/api/auth/signup', state);
    ...
    return (
        <div className="d-flex justify-content-center align-items-center vh-100">
            <form className="p-4 rounded bg-white shadow" onSubmit={handleSubmit} style={{ width: '100%', maxWidth: 400 }}>
                ...
                <div className="mb-3">
                    <label htmlFor="fullName" className="form-label fs-14 fs-md-6 fw-semibold">Full Name</label>
                    <input onChange={handleChange} value={state.fullName} name='fullName' type="text" className="form-control fs-14 fs-md-6" id="fullName" placeholder="Enter your full name" required />
                </div>
                ...
                <button type="submit" className="btn btn-primary w-100 mb-3 fw-semibold">Signup</button>

                <div className='d-flex justify-content-between' style={{ fontSize: 13 }}>
                    <p>Already have an Account?</p>
                    <Link href='/login' className='text-decoration-underline text-black'>Login</Link>
                </div>
            </form>
        </div>
    )
